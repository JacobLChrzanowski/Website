...
try:
    ...
    raise e
try:
    postgres = psycopg2.connect(host='inacanoe-qa-postgres', user=os.environ['POSTGRES_USER'], password=os.environ['POSTGRES_PASSWORD'])
    logging.info('psycopg2 could connect')
except psycopg2.OperationalError as e:
    logging.critical('psycopg2 failed to authenticate to postgres')
    raise e
#redis = Redis(host='redis', port=6379)
#postgres = psycopg2.connect(host='inacanoe-qa-redis', user=os.environ['POSTGRES_USER'], password=os.environ['POSTGRES_PASSWORD'])
